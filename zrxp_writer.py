...
class zrxp_writer:
        ...
        with open(file, 'w+') as fh:
            for key in zrxpdata:
                ...
                for ((y, m, d), val) in data:
                    ...
                    if val == '':
                        logging.error("write_wra_dict2zrxp. val=''. where key='"+key+" "+str([y, m, d]))
                    else:
                        val = float(val)
    ...
    """
        write_bom_storage_dfs2zrxp( path, station, 'level', importnumber, df_level)
        write_bom_storage_dfs2zrxp( path, station, 'volume', importnumber, df_volume)

    """

    def write_bom_storage_dfs2zrxp(self, outputpath, station, parameter, importNumber, df):
        
        zrxpfile = os.path.join(outputpath,   'bom-'+station+'_'+parameter+'.zrxp')
        with open(zrxpfile, 'w') as fh:
            self.write_header(fh, importNumber)
            for index, row in df.iterrows():
                if row['value']=='':
